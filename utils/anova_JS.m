function [p,anovatab,stats] = anova_JS(x,group,displayopt,extra)
...
if (classical)
   if (SSE~=0)
      F = (RSS/df1) / mse;
      p = fpval_JS(F,df1,df2);        % Probability of F given equal means.
...
ax = axes('Parent',f1);
if (~grouped)
   boxplot(ax,xorig,'notch','off'); %'BoxStyle','filled'
else
   boxplot(ax,xorig,groupnum,'notch','off'); %'BoxStyle','filled'
   h = get(ax,'XLabel');
