function [Complex_img, Mag_img,Phase_img,W,H] = Mac_for_NMF( param_real )
%%% initialization
[nx,ny,nt,nc]= (size(param_real.y));
AAA=(1-param_real.W1);
Weighting=AAA./max(AAA(:));
%% initialize
M = param_real.init_mag_low_channel;
P = param_real.init_phase_low_channel;
...
fprintf('\n pre-reconstruction ... \n');
%%% For initialization %%%
params.FT = param_real.FT; % multicoil model (b1: coil sensitivities)
params.W = param_real.W; % sparsifying transform
params.lambda=0.005;
params.dec = 0.01;
params.nite=param_real.pre_iter;
params.tol=5e-4;
params.display=param_real.display;
params.y = param_real.y;
params.scale = param_real.scale;
params.phase = P;
params.weighting = param_real.W1;
tic
M_tmp=CSL1SoftThresh_nmf_v1(M,params);
toc
M_update=M_tmp;
P_update=P;
...
%%% l2 norm CG problem %%%    
  if 1 < ite 
     if param_real.lambda_1
           [res,WH]= NMF_recon_single_coil_v3_104(M,param_real);
           M_NMF=res;
